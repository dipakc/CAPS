...
object ArrayProd extends DerivationScript {

    val name = "ArrayProd"

	def apply(): SynthTree = {
    	val logger= LoggerFactory.getLogger("progsynth.ArrayProd")
		...
		val post = s eqeq TimesQTermInt(i, c(0) <= i && i < N, arr.select(i))
		...
		.applyTacticBatch(new StepIntoUnknownProgIdxTactic(1))
		.applyTacticBatch(new IntroAssignmentTactic((s, one) :: Nil))
		...
		.applyTacticBatch(new InstantiateMetaTactic((prime(n), n + one) ::
				(prime(s), FnApp(TimesIntFn, List(s , arr.select(n)))) :: Nil))
