...
//cohen 154
object ArrayMin extends DerivationScript {

    val name = "ArrayMin"

	def apply(): SynthTree = {
    	val logger= LoggerFactory.getLogger("progsynth.ArrayMin")
		...
		//dummies
		val i = VarInt("i")
		val inf = arr.select(0)
		//pre and post
		val pre = TermBool.TrueT
		val post = m eqeq MinQTermInt(i, c(0) <= i && i < N, arr.select(i))
		...
		.applyTacticBatch(new RTVInPostTactic(N, n, c(1), c(1) <= n && n <= N))
		.applyTacticBatch(new StepIntoUnknownProgIdxTactic(1))
		.applyTacticBatch(new DeleteConjunctTactic(n eqeq N, N - n))
		.applyTacticBatch(new StepIntoUnknownProgIdxTactic(1))
		.applyTacticBatch(new IntroAssignmentTactic((m, arr.select(0)) :: Nil))
		...
		.applyTacticBatch(new InstantiateMetaTactic((prime(n), n + c(1)) ::
				(prime(m), FnApp(MinIntFn, List(m , arr.select(n)))) :: Nil))
