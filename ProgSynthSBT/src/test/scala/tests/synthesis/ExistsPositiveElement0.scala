...
class ExistsPositiveElement0 extends FunSuite with ShouldMatchers with PSPluginTesterDbg {
	ignore("ExistsPositiveElement. NOT YET IMPLEMENTED") {
		val inputFileName = """src\test\scala\tests\synthesis\ExistsPositiveElement0Src.scala"""
		val outputFileName = """tests.synthesis.ExistsPositiveElement0.html"""
		val (compilerOpt, runOpt, psc) = runCompiler( inputFileName :: Nil, Some(outputFileName))
		withClue ("compilerOpt && runOpt && psc isDefined:"){
			compilerOpt.isDefined && runOpt.isDefined && psc.isDefined should equal(true)
		}
